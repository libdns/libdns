...
// well-defined semantics for the purposes of reading and manipulating DNS
// records using DNS provider APIs.
//
...
// grants [Record] structs portability across zones. As such, record names are
// partially qualified, i.e. relative to the zone. For example, an “A” record
// called “sub” in zone “example.com.” represents a fully-qualified domain name
// (FQDN) of “sub.example.com.”. Implementations should expect that input
// records conform to this standard, while also ensuring that output records do;
// adjustments to record names may need to be made before or after provider API
// calls, for example, to maintain consistency with all other [libdns] packages.
// Helper functions are available in this package to convert between relative
// and absolute names.
//
...
// pair a zone's name with its records in a way that works for them.
//
...
// either both their expected outcomes or an error.
//
// For example, if [libdns.RecordAppender.AppendRecords] is called at the same
// time and two API requests are made to the provider at the same time, the
// result of both requests must be visible after they both complete; if the
// provider does not synchronize the writing of the zone file and one request
// overwrites the other, then the client implementation must take care to
// synchronize on behalf of the incompetent provider. This synchronization need
// not be global; for example: the scope of synchronization might only need to
// be within the same zone, allowing multiple requests at once as long as all of
// them are for different zones. (Exact logic depends on the provider.)
//
...
// reflected in the output. It may create or overwrite records or—depending on
...
// decision to support them in [libdns.RecordGetter.GetRecords], so end-users
// should not blindly call “SetRecords” on the output of
...
//
// “ListZones” returns the list of available DNS zones for use by other [libdns]
// methods.
//
// Implementations must honor context cancellation and be safe for concurrent
// use.
...
// # Type
//
// The [libdns.Record.Type] field specifies the type of the record as an
// uppercase string. Implementations may or may not support any given record
// type, and may support additional private record types with
...
// # Name
//
// The [libdns.Record.Name] field specifies the name of the record. It is
// partially qualified relative to the current zone. This field is called a
// “Label” by RFC 7719. You may use “@” to represent the root of the zone.
//
...
// # Value
//
// The [libdns.Record.Value] field specifies the value of the record. This field
// should be formatted in the standard zone file syntax, but should omit any
// fields that are covered by other fields in this struct.
//
// Examples:
//
//   - (A) “192.0.2.1”
...
//   - (CNAME) “example.com.”
//
//     Even though the value is traditionally called the “target”, it is
//     included only in the “Value” field here.
//
//   - (MX) “mail.example.com.”
//
//     Note that this excludes the priority field!
//
//   - (TXT) “Hello, world!”
//
//   - (SRV) “8080 example.com.”
//
//     Note that this excludes the priority and weight fields, but includes the
//     port. Also note that the target is included here, and not in the
//     [libdns.Record.Target] field.
//
//   - (HTTPS) “alpn=h2,h3 port=443”
//
//     Note that this excludes the priority field and target fields.
//
// # TTL
//
// The [libdns.Record.TTL] field specifies the time-to-live of the record. This
// is represented in the DNS as an unsigned integral number of seconds, but is
// provided here as a [time.Duration]. Fractions of seconds will be rounded down
// (aka truncated). A value of “0” means that the record should not be cached.
//
...
// # Priority
//
// The [libdns.Record.Priority] field specifies the priority of the record. This
// field is only applicable for certain record types, but is mandatory for those
// types.
//
// Examples:
//
//   - (MX) “10”
//
//     Note that this is traditionally called the “preference” in the DNS.
//
//   - (SRV) “10”
//
//   - (URI) “10”
...
// # Weight
//
// The [libdns.Record.Weight] field specifies the weight of the record. This
// field is only applicable for certain record types, but is mandatory for those
// types.
//
// Examples:
...
// # Target
//
// The [libdns.Record.Target] field specifies the target of the record. This
// field is only valid for “HTTPS” and “SVCB” records, and *not* for “SRV”,
// “MX”, or “CNAME” records, which store their targets in the
// [libdns.Record.Value] field. This field must be set to the fully-qualified
// domain name (FQDN) of the target (which includes the trailing dot).
//
// Examples:
//   - (HTTPS) “example.com.”
//   - (SVCB)  “example.com.”
...
// [Record.ToSRV] parses the record into a [SRV] struct with fully-parsed,
// literal values.
//
...
// [SRV.ToRecord] converts the parsed SRV data to a [Record] struct.
//
// EXPERIMENTAL; subject to change or removal.
...
// name *is* permitted here, and will be identically to “@”.
